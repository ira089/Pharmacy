...
const CartPage = () => {
  ...
  const cart = productsUser?.map(({idProduct, quantity }) => ({idProduct, quantity}))
  console.log(cart)
 ...
 const {total, totalQuantity} = totals
  ...
  const placeOrder = ({...formData, paymentMethod, total, totalQuantity, ...cart })
  console.log(placeOrder)
  return (
    <section className={styles.wrapCart}>
    <h2 className={styles.title}>Cart</h2>
    <CartInfo total={total} handleChange={handleChangeRadio} handleChangeForm={handleChangeForm}/>
    <CartProduts />
</section>
  )
}
